...
.setFooter('Requested by '+message.author.username, message.author.avatarURL)
.setDescription(`**لقد تم تغيير لونك ** ✅ `)
...
if(!message.channel.guild) return message.reply(' Error : \` Guild Command \`');
  var invites = await message.guild.fetchInvites();
    ...
    invites.forEach(i => {
      ...
      if (i.uses === 1) {//يمديك تعدل رقم وصول العدد حق الانفايت الى اأقل أو أكثر
          message.member.addRole(message.member.guild.roles.find("name","Vital"))//هنآ أسم ألرتبه اللي تجيهه
.catch(RebeL =>{
console.log('`Error`: ' + RebeL);
});
}
if (i.uses === 1) {
message.member.addRole(message.member.guild.roles.find("name","Vital"))
.catch(RebeL =>{
console.log('`Error`: ' + RebeL);
});
}
